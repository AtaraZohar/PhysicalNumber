...
int main() {
  ...
  if (signal == 0) {
    ...
    .CHECK_OUTPUT(check1,  "2.233333333[hour]")
    .CHECK_OUTPUT(check4+check3, "43.533333333[min]")
    .CHECK_OUTPUT(check7-check1, "0[sec]")
    .CHECK_OUTPUT(check1+check5+check7, "8.233333333[hour]")
  ...
  .setname("check length unit")
    ...
    .CHECK_OUTPUT(check16+check14, "3.01[cm]")
